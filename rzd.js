...
(function (angular) {
    ...
    return angular.module('rzd', ['ngResource', 'rzd_client_config']).value(
        'SEAT_TYPES', ['Плац', 'Купе', 'Люкс', 'Сид']
    ).value(
        'RZD_DATE_FORMAT', 'dd.MM.yyyy'
    ).value(
        'RZD_TIME_FORMAT', 'HH.mm'
    ).factory('stationsSuggester', ['$resource', 'GLOBAL_CONFIG',
    ...
    ).factory('trackedStationsLookup', ['$resource', 'GLOBAL_CONFIG',
        function($resource, GLOBAL_CONFIG) {
    ...
    ).factory('RZDLookup', ['$resource', 'GLOBAL_CONFIG',
        function($resource, GLOBAL_CONFIG) {
            return $resource(["http://",
                GLOBAL_CONFIG.api_host,
                GLOBAL_CONFIG.api_prefix,
    ...
    ).factory('StorageLookup', ['$resource', '$filter', 'RZD_DATE_FORMAT', 'RZD_TIME_FORMAT', 'GLOBAL_CONFIG',
        function($resource, $filter, RZD_DATE_FORMAT, RZD_TIME_FORMAT, GLOBAL_CONFIG) {
            return $resource(["http://",
                GLOBAL_CONFIG.api_host,
                ...
                fetchList: {
                    ...
                    transformResponse: function(data, headersGetter) {
                        ...
                        if (rows.length > 0) {
                            ...
                            now = Date.now();
                            current_date = $filter('date')(now, RZD_DATE_FORMAT);
                            current_time = $filter('date')(now, RZD_TIME_FORMAT),
    ...
    ]).factory('encodeDict', function() {
        return function (o, skip_enc, skip_empty_items) {
            var key, res = [];

            for (key in o) {
                if (!skip_empty_items || o[key]) {
                    res.push(encodeURIComponent(key) + '=' + (skip_enc && o[key] || encodeURIComponent(o[key])));
                }
            }

            return res.join('&')
        }
    }).service('trackingTask', ['$window', 'encodeDict', 'GLOBAL_CONFIG', function($window, encodeDict, GLOBAL_CONFIG) {
        var task_registry = {}, WSConnection, Watcher, Task;
        ...
        WSConnection = function () {
            ...
            constructor.prototype._connect = function () {
                var reconnect = this._reconnect.bind(this);
            ...
            constructor.prototype.send = function (msg) {
                ...
                if (this.ws.readyState == this.ws.CONNECTING) {
                    ...
                    throw Error('web socket is closed');
                }
            }

            return function () {
                if (_connection == undefined) {
                    _connection = new constructor();
                }
        ...
        Watcher = function(train_num, seat_type, car_num, seat_num, seat_pos) {
            this.input = {};
        ...
        Task.prototype.fallback = function(connection) {
            ...
            if (this.isActive()) {
                ...
                angular.forEach(this.watchers, function(watcher, w_key) {
        ...
        Task.prototype.addWatcher = function(train_num, seat_type, car_num, seat_num, seat_pos) {
            var w = new Watcher(train_num, seat_type, car_num, seat_num, seat_pos);
        ...
        Task.prototype._addWatcher = function (w, connection) {
            connection = connection || WSConnection();
        ...
        Task.prototype.removeWatcher = function(w_key, hardly) {
            if (w_key in this.watchers) {
                ...
                WSConnection().send(['unwatch', this.key, w_key].join(' '));
            }
        }
        ...
        Task.prototype.stop = function() {
            ...
            WSConnection().send(['remove', this.key].join(' '));
            delete task_registry[this.key];
            return this;
        }

        Task.prototype.inconsiderableMsgTmpl = RegExp('^(\\+W|\\-W|stopped)');
        ...
        Task.prototype.processReport = function(msg) {
            ...
            if (this.inconsiderableMsgTmpl.test(msg)) {
                ...
                if (this.state.waiting_for_result) {
                    ...
                    for (train_num in data) {
                        train_data = data[train_num];

                        train_data.watchers.forEach(function(w_key) {
                            console.log(w_key);
                        ...
                        this.on_success && this.on_success(train_num, train_data);
                    }
                } else if (this.isFailured()) {
                    this.result.errors = JSON.parse(msg);
                } else if (msg.length == 2) {
                    this.state.status = parseInt(msg[0]);
                    if (msg[1] == '-') {
